...
- **HTTP (streamable/SSE):**
  ...
  - Health: `/health` (no auth — the only path exempt from the bearer middleware), `/info`, `/metrics`, `/db_health` (auth)
  - OAuth 2.1 (no auth, present whenever a bearer token is configured): `/.well-known/oauth-authorization-server`, `/authorize`, `/token`, `/register`
- **Cloudflare tunnel:** `legacymind-mcp` → https://mcp.samataganaphotography.com/mcp (token required).
- **Database:** SurrealDB 3.x at `127.0.0.1:8000` (set `SURR_DB_URL` as bare `host:port` — the `Ws` engine adds the scheme and the `/rpc` path itself; a `/rpc` suffix is never a valid input), user `root`/`root`; SurrealMind default namespace/database is `surreal_mind` / `consciousness`. Legacy namespaces may still exist after migrations.
