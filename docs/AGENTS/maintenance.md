...
- **Maintenance subprocess timeout:** `maintenance_ops` accepts optional `timeout_ms` for `rethink`, `consolidate`, `populate`, `embed`, `wander`, `health`, and `tasks`; it defaults to 1,800,000 ms (30 minutes) and accepts 100–3,600,000 ms. This subprocess supervisor is independent of `SURR_HTTP_REQUEST_TIMEOUT_MS`, `SURR_HTTP_MCP_OP_TIMEOUT_MS`, and `SURR_TOOL_TIMEOUT_MS`.
