...
| `search` | Unified KG + thoughts retrieval. Params: `target` (`entity\|relationship\|observation\|mixed`), `include_thoughts`, `thoughts_content`, `top_k_memories`, `top_k_thoughts`, `sim_thresh`, `confidence_[g\|l]te`, `date_from/to`, `order`, continuity filters. Supports direct ID lookup via `query.id`. |
| `remember` | Create KG `entity\|relationship\|observation`. Supports `upsert`, `source_thought_id`, `confidence`, `data`. |
| `wander` | Explore the knowledge graph serendipitously. Modes: `random`, `semantic`, `meta`. Returns actionable guidance for improving KG quality. |

## Maintenance & Help

| Tool | Description |
|------|-------------|
| `maintain` | System maintenance subcommands: `health_check_embeddings`, `health_check_indexes`, `reembed`, `reembed_kg`, `list_removal_candidates`, `export_removals`, `finalize_removal`, `ensure_continuity_fields`, `echo_config`. |
