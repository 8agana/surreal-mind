...
- Inner Voice: defaults `mix=0.6`, `topk_default=10`, `min_floor=0.15`, `max_candidates_per_source=150`, planner off by default (`SURR_IV_PLAN`). Recency: half-life default 14d (`SURR_IV_RECENCY_HALF_LIFE_DAYS`), optional window (`SURR_IV_RECENCY_DEFAULT_DAYS`), bias flag (`SURR_IV_PREFER_RECENT`).
