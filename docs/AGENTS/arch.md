...
- Treat inherited environment variables and values loaded from `.env` as trusted operator configuration. `dotenvy` merges file values into the process-global environment; after that merge, `std::env::var` cannot identify whether a value came from a shell, launchd, a local `.env`, or an ancestor `.env`.
