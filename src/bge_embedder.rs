...
impl BGEEmbedder {
    pub fn new() -> Result<Self> {
        ...
        #[cfg(target_os = "macos")]
        let device = {
            let use_metal = std::env::var("SURR_USE_METAL").ok().map(|v| v != "false" && v != "0").unwrap_or(true);
            if use_metal {
...
impl Embedder for BGEEmbedder {
    async fn embed(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let instructed_text = format!("Represent this sentence for searching relevant passages: {}", text);
        ...
        let embeddings = self.model.forward(&input_ids, &token_type_ids, Some(&attention_tensor))?;
