...
async fn main() -> Result<()> {
    ...
    let mut batch_size: usize = env::var("REEMBED_BATCH")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(64);
    let mut limit: Option<usize> = env::var("REEMBED_LIMIT").ok().and_then(|v| v.parse().ok());
    let mut missing_only: bool = env::var("REEMBED_MISSING_ONLY")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(true);
    let mut dry_run: bool = env::var("REEMBED_DRY_RUN")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false);
    ...
    eprintln!(
        "Re-embedding thoughts: batch_size={}, limit={:?}, missing_only={}, dry_run={}",
        batch_size, limit, missing_only, dry_run
    );
