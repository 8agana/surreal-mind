...
async fn main() -> Result<()> {
    ...
    let res: Vec<serde_json::Value> = db.query("SELECT count(), extracted_to_kg FROM thoughts GROUP BY extracted_to_kg").await?.take(0)?;
    println!("Extraction distribution: {:?}", res);
