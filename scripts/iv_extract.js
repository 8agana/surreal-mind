...
async function run() {
  ...
  try {
    ...
    console.error(
      JSON.stringify({ stage: "error", reason: "invalid_input_json" }),
    );
    process.exit(2);
  }
  ...
  const models = (
    process.env.IV_CLI_MODELS ||
    process.env.IV_MODELS ||
    ...
    .filter(Boolean);
  const cliCmd =
    process.env.IV_CLI_CMD || process.env.IV_SYNTH_CLI_CMD || "gemini";
  const cliArgsJson =
    process.env.IV_CLI_ARGS_JSON ||
    process.env.IV_SYNTH_CLI_ARGS_JSON ||
    '["-m","{model}"]';
  let cliArgs;
  try {
  ...
  const strictJson =
    (process.env.IV_STRICT_JSON || "false").toLowerCase() !== "false";
  const perTimeoutMs = parseInt(process.env.IV_TIMEOUT_MS || "20000", 10);
  const totalTimeoutMs = parseInt(
    process.env.IV_TOTAL_TIMEOUT_MS || "30000",
    10,
  );
  const allowGrok =
    (process.env.IV_ALLOW_GROK || "true").toLowerCase() !== "false";
  ...
  for (const model of models) {
    ...
    let truncated = false;
    if (e.length > maxEntities) {
      obj.entities = e.slice(0, maxEntities);
      truncated = true;
    }
    if (r.length > maxEdges) {
      obj.edges = r.slice(0, maxEdges);
      truncated = true;
    }
    ...
    if (!valid.ok) {
      console.error(
        JSON.stringify({
          ...
          provider: model,
          status: "fail",
          reason: valid.reason || "schema_invalid",
        }),
      );
      continue;
    }
    ...
    console.error(
      JSON.stringify({
        stage: "result",
        provider: model,
        entities: (obj.entities || []).length,
        edges: (obj.edges || []).length,
  ...
  console.error(
    JSON.stringify({ stage: "fatal", reason: "no_provider_succeeded" }),
  );
  process.exit(3);
}
...
function runCli(cmd, args, prompt, timeoutMs) {
  return new Promise((resolve) => {
    ...
    child.on("close", (code) => {
      clearTimeout(timer);
      if (code !== 0)
        return resolve({ ok: false, reason: "non_zero_exit", code, err });
  ...
  const messages = [
    {
      role: "system",
      content:
        "Output ONLY one JSON object matching the required schema. No prose.",
    },
...
run().catch((e) => {
  console.error(
    JSON.stringify({ stage: "fatal", reason: e.message || "unknown" }),
  );
  process.exit(1);
});
